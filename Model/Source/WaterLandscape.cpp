...
void WaterLandscape::calculateProcesses(int year, int day, Weather* w) {
	...
	for (int hour = 0; hour < hoursPerDay; hour++){
        ...
        //---------------------------------------------
		if (hour == 23) {
                /// \implements \fn WaterLandscape::potEvapotranspiration(int year, int day, Weather* w)
...
void WaterLandscape::setElevation(Parameter* p) {
	...
	parameterFileName = "Parameters" + delimiter + "elevation_" + std::to_string(xsize) + "_" + p->elevationFileID[p->scenario] + ".txt";
...
void WaterLandscape::initialiseResultFile(Parameter* p) {
    ...
    //create and open result file
    string resultFileNameDay = "Results"+ delimiter + p->site + "_" + std::to_string(p->simYears) + "years_daily_" +
