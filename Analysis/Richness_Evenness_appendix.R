...
richperclimreptype <- sumperyear %>%
  group_by(scenario, type, climrep) %>%
  summarise_at(vars(Richness), funs(speciesRich = mean)) # only consider mean of last 20 years of simulation (and all climreps)
...
richperclimrepscen <- richperclimreptype %>%
  ...
  summarise_at(vars(speciesRich), funs(speciesRich = sum)) # only consider mean of last 20 years of simulation (and all climreps)
