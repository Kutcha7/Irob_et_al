...
void Shrub::basicMortality(VegetationCell* vegetation, int index) {
	...
	basicM = 0;
		...
		if (vegetation->cohortAge[index] > 40){//_dirk_10092010
            ...
            double rn = rand() / (double(RAND_MAX) + 1);
			rn = rn + 0.5;
	...
	double rn = rand() / (double(RAND_MAX) + 1); /// \todo Dirk: Check whether this is valid code for random number generation
		if (rn < vegetation-> gBasicMortRate_min && vegetation->cohortAge[index] > 40){ 
			basicM =  Cover ;
		}
...
void Shrub::vegMortality(VegetationCell* vegetation, int day, int growStart, int growEnd, int year, Weather * weather) {
	...
	double randomNumber = rand() / (double(RAND_MAX) + 1);
