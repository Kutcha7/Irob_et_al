 ...
 * \todo Functions calculateUptakeRateCover and calculateWaterUptake will be called every day,
 * so why calling various time in several- if-statements and not at the beginning once?
 * is called in --> controller::runSimulation
 *******************************************************************************************/
void VegetationLandscape::calculateProcesses(int year, int day, WaterLandscape* waterlandscape, Weather* we, Parameter* p){  /*Tong, based on PFT-oriented*/
    ...
    if (day == 0) {
        ...
        ///----------------------------END: FIRST DAY OF THE FIRST YEAR---------------------------------

        for (int m = 0; m < Shrubs; m++){
            estCount[m] = 0;
        }

        firemem = fire2; // Übertrage fire2 Wert auf fire Mem, so dass man weiss ob im Vorjahr feuer war!
        fire2 = false;

        for (int l = 0; l < 10; l++) {  //count of cells above 10 levels is set to zero
        ...
        //fire is initiated at the end of the dry season, before growth period starts, hence plants have the possibility to grow hereafter, but not so, shrubs..?
        //fire_ini
        ...
        for (int x = 0; x < xsize; x++) {
            for (int y = 0; y < ysize; y++) {
                for (int i = 0; i < PFTs; i++){
                    /// \implements \fn PlantFunctionalType::calculateUptakeRateCover(VegetationCell* vegetation, Weather* weather, int year)
                    veggrid[x][y]->pftList[i]->calculateUptakeRateCover(veggrid[x][y], we, year);
                }
	...
	///----------------------------END: FIRST DAY OF THE YEAR-------------------------------------------

   ///----------------------------EVERY VEGTIMESTEP WITHIN WET SEASON-------------------------------------------
    if (day >= wetStart && day <= wetEnd && day%vegTimeStep == vegTimeStep-1){ 
        for (int x = 0; x < xsize; x++){
            for (int y = 0; y < ysize; y++){
                ...
                //-----------------------------------------------------------------
                for (int i = 0; i < PFTs; i++){
                    /// \implements \fn PlantFunctionalType::calculateAvMoistL1(VegetationCell* vegetation)
                    veggrid[x][y]->pftList[i]->calculateAvMoistL1(veggrid[x][y]);
                    /// \implements \fn PlantFunctionalType::calculateAvMoistL2(VegetationCell* vegetation)
                ...
                //--------------------------------------
                for (int i = 0; i < PFTs; i++){
                    /// \implements \fn PlantFunctionalType::calculateGrowthL1(VegetationCell* vegetation, int index)
                    veggrid[x][y]->pftList[i]->calculateGrowthL1(veggrid[x][y], i);
                    /// \implements \fn PlantFunctionalType::calculateGrowthL2(VegetationCell* vegetation, int index)
                    ...
                    /// \implements \fn PlantFunctionalType::calculateBiomass(VegetationCell* vegetation, Weather* we2, int yr2)
                    veggrid[x][y]->pftList[i]->calculateBiomass(veggrid[x][y], we, year);
                }
				...
				///----------------------------END: EVERY VEGTIMESTEP WITHIN WET SEASON-------------------------------------------
		}
    }
    
    ///----------------------------END: WITHIN WET SEASON-------------------------------------------

    ///----------------------------AT THE END OF THE GROWING SEASON-------------------------------------------
    //if the soil was relatively dry during the growth season: mortality due to water shortage (vegMortality())
	// a basic stochastic mortality  due to several possible but not explicitly simulated reasons (frost, disease, browse)
    if (day == growEnd){

        //--------------------------------------------------------------------------------------------
        /// CALCULATE WATER UPTAKE RATE, VEGMORTALITY, BASIC MORTALITY, BASIC MORTALITY RATE, BIOMASS.
        //--------------------------------------------------------------------------------------------
        for (int x = 0; x < xsize; x++) {
            for (int y = 0; y < ysize; y++) {
                for (int i = 0; i < PFTs; i++){
                    if (i < Shrubs) {
                        if (veggrid[x][y]->pftList[i]->Cover > 0) {
                            veggrid[x][y]->cohortAge[i]++;  // increase age of shrub cohort by 1
                            veggrid[x][y]->cohortAgeFire[i]++;  // increase age of shrub cohort by 1
                        } else {
                ...
                for (int i = 0; i < PFTs; i++){
                    /// \implements \fn PlantFunctionalType::vegMortality(VegetationCell* vegetation, int day, int growStart, int growEnd, int year, Weather* weather)
                    veggrid[x][y]->pftList[i]->vegMortality(veggrid[x][y], day, growStart, growEnd, year, we);
                }

                veggrid[x][y]->gBasicMortRate_min = veggrid[x][y]->pftList[Shrubs]->BasicMortRate;
                for (int i = 0; i < PFTs; i++){
                    /// \implements \fn PlantFunctionalType::basicMortality(VegetationCell* vegetation, int index)
                    veggrid[x][y]->pftList[i]->basicMortality(veggrid[x][y], i);

                    if (i >= Shrubs && i < Perennials) {
                        if (veggrid[x][y]->pftList[i]->BasicMortRate < veggrid[x][y]->gBasicMortRate_min ){
                            veggrid[x][y]->gBasicMortRate_min = veggrid[x][y]->pftList[i]->BasicMortRate;
                        }
                    }
                    /// \implements \fn PlantFunctionalType::calculateBiomass(VegetationCell* vegetation, Weather *we2, int yr2)
                    veggrid[x][y]->pftList[i]->calculateBiomass(veggrid[x][y], we, year);
    ...
    if (day == 364) {
        ...
        /// \implements \fn VegetationLandscape::writeYearlySummary(WaterLandscape *waterlandscape, Weather *we, int year)
        //----------------------------------------------------------------------------------------------------------
 ...
 *******************************************************************************************/
void VegetationLandscape::calculateShrubDispersal(int x1, int y1, int x2, int y2){  /*Tong, based on PFT-oriented*/
	...
	//dirk faktor von Stocking Rate erhoeht ausbreitungsdistanz und flacht Kurve ab
	for (int i = 0; i < Shrubs; i++){
        newdistconst[i] = veggrid[x1][y1]-> pftList[i]->distConstS + veggrid[x1][y1]->pftList[i]->distConstS2 * stockingRate;
	}
	//euklidian distance
	...
	double zuf = rand() / (double(RAND_MAX) + 1);  //random effect of dispersal (maybe zero, maybe double of calculated value)

	for (int i = 0; i < Shrubs; i++){
		dispHelpS[i] = veggrid[x1][y1]->pftList[i]->disFrac * veggrid[x1][y1]-> pftList[i]->dist0S * exp(-newdistconst[i] * dist) * veggrid[x1][y1]->pftList[i]->Cover * veggrid[x2][y2]->pftList[i]->est * max((1 - scover - pcover),0.0);
 ...
 *******************************************************************************************/
void VegetationLandscape::dispersal(int Stockrate){ /*Tong, based on PFT-oriented*/
    ...
    for (int x = 0; x < xsize; x++){
        for (int y = 0; y < ysize; y++){
            for (int i = Shrubs; i < Shrubs + Perennials; i++){
                ...
                dispHelp[i] += veggrid[x][y]->pftList[i]->disFrac * veggrid[x][y]->pftList[i]->Cover;
            }
        }
    }
    ...
    //assumption: grass is not limited in dispersal  //the amount per cell depends on the mean cover of the total grid
    for (int x = 0; x < xsize; x++){
        for (int y=0; y<ysize; y++) {
            ...
            for (int i = Shrubs; i < Shrubs + Perennials; i++){
                veggrid[x][y]->pftList[i]->DispSend = veggrid[x][y]->pftList[i]->Cover * veggrid[x][y]->pftList[i]->disFrac;
            }

            double zuf = rand() / (double(RAND_MAX) + 1);

            //receiving part of dispersal seeds of perennial grass
            for (int i = Shrubs; i < Shrubs + Perennials; i++){
                veggrid[x][y]->pftList[i]->Disp = dispHelp[i] * veggrid[x][y]->pftList[i]->est * max( 1 - gcover - scover,0.0);
                if (zuf < 0.01){
                    veggrid[x][y]->pftList[i]->Disp += 0.02/Perennials;
                }
            }
        }
    }
    ...
    /*----------------------- dispersal of shrubs ------------------------*/
	// neue "distablishment rule vom 15.04.2010 mit neuer SR Anpassung vom 5.Mai2010
	//dirk faktor von Stocking Rate erhoeht ausbreitungsdistanz und flacht Kurve ab
    ...
    for (int x1 = 0; x1 < xsize; x1++) {
        for (int y1 = 0; y1 < ysize; y1++) {
            for (int i = 0; i < Shrubs; i++){
                newdistconst[i] = veggrid[x1][y1]->pftList[i]->distConstS + veggrid[x1][y1]->pftList[i]->distConstS2 * Stockrate;
                double ExitParam = veggrid[x1][y1]->pftList[i]->dist0S * maxDistS; //maxdistS% von VMax
                int helpDist = 0;
                double helpMaxDist;

                do {
                    helpMaxDist =  veggrid[x1][y1]->pftList[i]->dist0S * exp(-newdistconst[i] * helpDist);
                    helpDist++;
                } while (helpMaxDist > ExitParam);

                //square around x1 and y1 (within distance), with boundary  only for cells with shrubs that are "adult"
				if (veggrid[x1][y1]->pftList[i]->Cover > scrlim){ //_dirk if cover below scrlim, we assume shrubs to be juvenile and non-reproducing
                    for (int x2 = max(x1 - int((helpDist) / (1.0 * cellsize)),0); x2 < min(x1 + int((helpDist) / (1.0 * cellsize)),xsize - 1); x2++) {
                        for (int y2 = max(y1 - int((helpDist) / (1.0 * cellsize)),0); y2 < min(y1 + int((helpDist) / (1.0 * cellsize)),ysize - 1); y2++) {

                            //---------------------------------------------
                            /// CALCULATE SHRUB DISPERSAL.
                            /// \implements \fn VegetationLandscape::calculateShrubDispersal(int x1, int y1, int x2, int y2)
                            //---------------------------------------------
    ...
    for (int x = 0; x < xsize; x++) {
        for (int y = 0; y < ysize; y++) {
            ...
            //  limit dispersal of each PFT if potential dispersal is bigger than left space
            if (coveradded > coverleft){
                for (int i = 0; i < Shrubs + Perennials; i++){
                    veggrid[x][y]->pftList[i]->Cover += (veggrid[x][y]->pftList[i]->Disp/coveradded) * coverleft;
                }
            }
...
void VegetationLandscape::meanCoverBiomass(){/*Tong, based on PFT-oriented*/
    ...
    for (int x = 0; x < xsize; x++) {
        for (int y = 0; y < ysize; y++) {
            for (int i = 0;i < PFTs;i++){
				...
				netMortR[i] += (veggrid[x][y]->pftList[i]->totalDryMort +  veggrid[x][y]->pftList[i]->basicM)  ;//  _dirk
            }
    ...
    for (int i = 0; i < PFTs; i++){
        meanB[i] = meanB[i] / ((xsize*ysize*cellsize*cellsize)/(100.0*100));
    }
 ...
 * is called in --> VegetationLandscape::calculateProcesses
 *******************************************************************************************/
void VegetationLandscape::grazing(Weather* we, int yr2) { //grazing algorithm has been rewritten by Tong

    //one purpose: remove the facilitation relation among PFTs (such as competition and coexistence relation in one function)
	//calculate the relative preference for each plant type, this parameter will be used to adjust the grazed amount
    //if we have a type that is preferred above average, relprefer[PFT] is more than 1; if it is below average, relprefer[PFT] is less than 1.
    for ( int i = 0; i < xsize;i++) {
        for ( int j = 0; j < ysize;j++)	{
	        ...
	        for (int m = 0; m < PFTs; m++){
                veggrid[i][j]->relprefer[m] = veggrid[i][j]->pftList[m]->grazeprefer / meanprefer;
	        }
        }
    }

    //calculate the needed biomass dependent on livestock density
    realstock = stockingRate; // realized stocking rate might change if not enough fodder is available
    double BMneed = 0;
    BMneed = 365 * percapability * BiomassNeed * (double)(1.0 / realstock);   // Total biomass need per year and ha (what one LSU needs per per year is 365*0.020*450000 )

    //calculate the average needed biomass per grid cell
    double avBMneed = 0.0;
    double areagrazed = (double) (xsize*cellsize*ysize*cellsize)/(100.0*100);  // area grazed in ha
    ...
    for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize; j++) {
            for (int m = 0; m < PFTs; m++){
                ...
                veggrid[i][j]->pftList[m]->Biomassold = veggrid[i][j]->pftList[m]->Biomass;
                // calculate the safe part of alive biomass
                veggrid[i][j]->safeBM_alive[m] = veggrid[i][j]->pftList[m]->grazeLim * veggrid[i][j]->pftList[m]->Biomass;
                // calculate the edible part of alive biomass
                veggrid[i][j]->edibleBM_alive[m] = veggrid[i][j]->pftList[m]->Biomass - veggrid[i][j]->safeBM_alive[m];
            }
        }
    }
        ...
        for (int j = 0; j < ysize;j++)	{
            ...
            for (int m = 0; m < PFTs; m++) {
                ...
                if (m < Shrubs) {
                    ...
                    // calculate the total edible biomass in a cell for for one annual grass
                    veggrid[i][j]->edibleBM_total[m] = veggrid[i][j]->edibleBM_reserve[m] +  veggrid[i][j]->edibleBM_alive[m];
                }
                else {
            ...
            //calculate the edible alive biomass for all shrubs in a cell, there is no reserve biomass for shrub
            veggrid[i][j]->edibleShrub = veggrid[i][j]->ShrubaliveBM;
            // calculate the total edible biomass in a cell for all annual grasses
            veggrid[i][j]->edibleAnnual = veggrid[i][j]->AnnualresBM + veggrid[i][j]->AnnualaliveBM;
            // calculate the total edible biomass in a cell for all perennial grasses
            veggrid[i][j]->ediblePerennial = veggrid[i][j]->PerennialresBM + veggrid[i][j]->PerennialaliveBM;

            //calculate the edible reserve biomass of annual grass and perennial grass in a cell
            veggrid[i][j]->reserveBM = veggrid[i][j]->AnnualresBM + veggrid[i][j]->PerennialresBM;
            //calculate the edible alive biomass of annual grass, perennial grass and shrub in a cell
            veggrid[i][j]->aliveBM = veggrid[i][j]->AnnualaliveBM + veggrid[i][j]->PerennialaliveBM + veggrid[i][j]->ShrubaliveBM;
            //calculate the total edible biomass in a cell
            veggrid[i][j]->totalBM = veggrid[i][j]->reserveBM + veggrid[i][j]->aliveBM;
        }
    }
    ...
    //if biomass needed is less than available biomass, then determine how much is taken from each grid cell and each type
   ...
   //if biomass needed is more than total edible biomass, livestock density should be adjusted
    int st = stockingRate;
    if (BMtotal < BMneed ) {
        double need=0;
        do{
            need = 365 * percapability * BiomassNeed * (double) (1.0/st);   // what they need per year and ha
            need = areagrazed * need;
            st++;
        } while (need > BMtotal);
        realstock = st;

        //remove edible part of reserve biomass and alive biomass except for safe biomass in all cells
        for (int i = 0; i < xsize;i++) {
            for (int j = 0; j < ysize;j++) {
                for (int m = 0; m < PFTs; m++){
                    if (m < Shrubs) {
        ...
        do {
            count++;
            zufx = (rand() % xsize); //random(xsize);
            zufy = (rand() % ysize); //random(ysize);   // individual cells are chosen randomly and from every cell biomass is removed
            demand_help = avBMneed;  // calculate demand of specific cell
            //first determine how much is eaten by each type and then how much of this comes from the reserve and alive biomass.
            //Reasoning: if cattle does not like type 1, then it will also not like the reserve biomass of type 1

            double edibleBMG = 0;
            double edibleBMA = 0;
            double edibleBMS = 0;

            //after grazing once, the reserve biomass and alive biomass of PFTs in one cell have changed, so should recalculate the edible part of PFTs
            for (int m = 0; m < PFTs; m++){
                if (m < Shrubs) {
                    veggrid[zufx][zufy]->edibleBM_alive[m] = veggrid[zufx][zufy]->pftList[m]->Biomass * (1-veggrid[zufx][zufy]->pftList[m]->grazeLim);
                    edibleBMS += veggrid[zufx][zufy]->edibleBM_alive[m];
                }
                else if (m >= Shrubs && m < Shrubs + Perennials) {
                    veggrid[zufx][zufy]->edibleBM_total[m] = (veggrid[zufx][zufy]->pftList[m]->resBM + veggrid[zufx][zufy]->pftList[m]->Biomass) * (1-veggrid[zufx][zufy]->pftList[m]->grazeLim);
                    veggrid[zufx][zufy]->edibleBM_reserve[m] = veggrid[zufx][zufy]->pftList[m]->resBM * (1-veggrid[zufx][zufy]->pftList[m]->grazeLim);
                    ratio_Reserve[m] = veggrid[zufx][zufy]->edibleBM_reserve[m] / veggrid[zufx][zufy]->edibleBM_total[m];
            ...
            for (int m = 0; m < PFTs; m++){
                if (veggrid[zufx][zufy]->ediblePerennial > 0 || veggrid[zufx][zufy]->edibleAnnual > 0 || veggrid[zufx][zufy]->edibleShrub > 0) {
                    if (m < Shrubs) {
                        reldemand[m] = veggrid[zufx][zufy]->relprefer[m] * (veggrid[zufx][zufy]->edibleBM_alive[m] / veggrid[zufx][zufy]->totalBM);
                    }
                    else {
                        reldemand[m] = veggrid[zufx][zufy]->relprefer[m] * (veggrid[zufx][zufy]->edibleBM_total[m] / veggrid[zufx][zufy]->totalBM);
                    }
                } else
                    reldemand[m] = 0;
            }
	   		...
	   		for (int m = 0; m < PFTs; m++){
                if (m < Shrubs) {
                    if (veggrid[zufx][zufy]->edibleBM_alive[m] > (reldemand[m] * demand_help)){
                        grazedPftBM[m] += (reldemand[m] * demand_help);
                        grazedBM += (reldemand[m] * demand_help);
                        veggrid[zufx][zufy]->edibleBM_alive[m] -= (reldemand[m] * demand_help);
                        veggrid[zufx][zufy]->pftList[m]->Biomass = (veggrid[zufx][zufy]->pftList[m]->Biomass * veggrid[zufx][zufy]->pftList[m]->grazeLim + veggrid[zufx][zufy]->edibleBM_alive[m]);
                    }
                        ...
                        veggrid[zufx][zufy]->pftList[m]->Biomass = veggrid[zufx][zufy]->pftList[m]->Biomass * veggrid[zufx][zufy]->pftList[m]->grazeLim;
                    }
                }
                else {
                    // if the sum of edible reserve biomass and edible alive biomass is more than biomass need, biomass need will be distributed based on the ratio of edible reserve biomass and edible alive biomass, applicable on perennial grass and annual grass
                    if (veggrid[zufx][zufy]->edibleBM_total[m] > (reldemand[m] * demand_help)){
                        grazedPftBM[m] += (reldemand[m] * demand_help) ;
                        grazedBM += (reldemand[m] * demand_help);
                        veggrid[zufx][zufy]->pftList[m]->resBM -= (reldemand[m] * demand_help * ratio_Reserve[m]);
                        veggrid[zufx][zufy]->pftList[m]->Biomass -= (reldemand[m] * demand_help * (1 - ratio_Reserve[m]));
                    }
                    // if not, edible reserve biomass and edible alive biomass will be removed, and left safe reserve biomass and safe alive biomass, applicable to annual grass and perennial grass
                    else {  // in this cell, after one grazing, annual grass should not be grazed any more
                        grazedPftBM[m] += veggrid[zufx][zufy]->edibleBM_total[m];
                        grazedBM += veggrid[zufx][zufy]->edibleBM_total[m];
                        veggrid[zufx][zufy]->pftList[m]->resBM = veggrid[zufx][zufy]->pftList[m]->resBM * veggrid[zufx][zufy]->pftList[m]->grazeLim;
                        veggrid[zufx][zufy]->pftList[m]->Biomass = veggrid[zufx][zufy]->pftList[m]->Biomass * veggrid[zufx][zufy]->pftList[m]->grazeLim;
                    }
                }
            }
        ...
        for (int i = Shrubs; i < Shrubs + Perennials; i++){
            if (isnan(veggrid[zufx][zufy]->pftList[i]->Biomass)) {
                cout <<"Grazing4" <<  "\t" << veggrid[zufx][zufy]->pftList[i]->Biomass << "\n";
            }
        }
    }
    //calculate grazing biomass for each PFT per ha
    grazedBM = (double)grazedBM / ((double)(xsize*ysize*cellsize*cellsize)/(100.0*100));
    for (int i = 0; i < PFTs; i++){
        grazedPftBM[i] = (double)grazedPftBM[i] / ((double)(xsize*ysize*cellsize*cellsize)/(100.0*100));
    }
    ...
    for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize;j++) {
            for (int m = 0; m < PFTs; m++){
                if (veggrid[i][j]->pftList[m]->Biomassold == 0) fraction[m] = 0;
                else fraction[m]  = (double)(veggrid[i][j]->pftList[m]->Biomassold - veggrid[i][j]->pftList[m]->Biomass) / veggrid[i][j]->pftList[m]->Biomassold;

                if (fraction[m] < 0) {
                                        cout << "Grazing fraction zu klein at x:" << i << " y: "<< j << "\t" << veggrid[i][j]->pftList[m]->Name <<"\t"<<

                    veggrid[i][j]->pftList[m]->Biomassold << "\t" << veggrid[i][j]->pftList[m]->Biomass<< "\t" << fraction[m]<< "\n";
                                        cout << "Grazing fraction zu klein at x:" << i << " y: "<< j << "\t" << veggrid[i][j]->pftList[0]->Name <<"\t"<< veggrid[i][j]->pftList[0]->Biomassold << "\t" << veggrid[i][j]->pftList[0]->Biomass<< "\t" << fraction[m]<< "\n";
                                        cout << "Grazing fraction zu klein at x:" << i << " y: "<< j << "\t" << veggrid[i][j]->pftList[1]->Name <<"\t"<< veggrid[i][j]->pftList[1]->Biomassold << "\t" << veggrid[i][j]->pftList[1]->Biomass<< "\t" << fraction[m]<< "\n";
                                        cout << "Grazing fraction zu klein at x:" << i << " y: "<< j << "\t" << veggrid[i][j]->pftList[2]->Name <<"\t"<< veggrid[i][j]->pftList[2]->Biomassold << "\t" << veggrid[i][j]->pftList[2]->Biomass<< "\t" << fraction[m]<< "\n";
                                }
                /// \implements \fn PlantFunctionalType::calculateCover(VegetationCell* vegetation, Weather* we2, int yr2, double factor)
                veggrid[i][j]->pftList[m]->calculateCover(veggrid[i][j], we, yr2, fraction[m]);
            }
            //calculate the cover of reserve part for annual grass and perennial grass
    ...
    // plant reserve biomass is calculated after grazing, their biomass contributes to a certain fraction to the fodder(Reserve biomass)
    for (int i = 0; i < xsize;i++) {
        for (int j = 0; j < ysize;j++) {
            for (int m = 0; m < PFTs; m++){
                if ( m < Shrubs) {
                    ...
                    veggrid[i][j]->pftList[m]->resBM *= veggrid[i][j]->pftList[m]->reserveLast;
                    //reserve biomass receives from the biomass of this year
                    veggrid[i][j]->pftList[m]->resBM += veggrid[i][j]->pftList[m]->reserveThis * veggrid[i][j]->pftList[m]->Biomass;
                    // calculate the safe part of reserve biomass
                    veggrid[i][j]->safeBM_reserve[m] = veggrid[i][j]->pftList[m]->grazeLim * veggrid[i][j]->pftList[m]->resBM;
                    // calculate the edible part of reserve biomass
                    veggrid[i][j]->edibleBM_reserve[m] = veggrid[i][j]->pftList[m]->resBM - veggrid[i][j]->safeBM_reserve[m];
                }
            }
 	 	}
    }
...
/*******************************************************************************************
 * calculate biodiversity indices (Richness, ...) including total PFTs present in the landscape *Katja*
 * will add one to Richness if x% cover are found in at least 1 cell
 * is called in --> VegetationLandscape::calculateBiodiv
 *******************************************************************************************/
...
int VegetationLandscape::calculateRichness() {
    ...
    for(int i =0; i<PFTs; i++) {
        ...
        for(int x = 0; x < xsize; x++) {
            for(int y=0; y < ysize; y++) {
                if (veggrid[x][y]->pftList[i]->Cover > 0.02) {
                    Richness ++ ;
                    return;
                }
            }
    ...
    double totalPFTcover{0};
        ...
        for (int i=0; i < PFTs; i++) {
            if (relativePFTcover[i]>0) //
                ShannonDiv += relativePFTcover[i]/totalPFTcover * log(relativePFTcover[i]/totalPFTcover);
        }
...
/*******************************************************************************************
 * calculate maximum, minimum and standard deviation of shrub cover for the whole grid
 * is called in --> VegetationLandscape::calculateProcesses
 *******************************************************************************************/
void VegetationLandscape::calculateShrub(){ /*Tong, calculate the maximum, minimum of shrub cover in each cell*/
 ...
 * is called in --> VegetationLandscape::calculateProcesses
 *******************************************************************************************/
void VegetationLandscape::calculateMean(int day) {/*Tong, based on PFT-oriented*/
    ...
    for (int x = 0; x < xsize; x++) {
        for (int y = 0; y < ysize; y++) {
            for (int i = 0; i < PFTs; i++){
                ...
                sumGrowth[i] += (veggrid[x][y]->pftList[i]->GrowthL1 + veggrid[x][y]->pftList[i]->GrowthL2);
            }
    ...
    if (meanRCover < 0.01) {
        meanRCover = 0;
    } else meanRCover = meanRCover / (numcells);
    ...
    for (int i = 0; i < PFTs; i++){
    	...
    	if (i < Shrubs) meanStotalcover += meanCover[i];
    	else if (i >= Shrubs && i < Shrubs + Perennials) meanGtotalcover += meanCover[i];
    	else meanAtotalcover += meanCover[i];
    }
    ...
    for (int x = 0; x < xsize; x++) {
        for (int y = 0; y < ysize; y++) {
            for (int i = 0; i < Shrubs; i++){
                sumshrub[i] += pow(fabs(veggrid[x][y]->pftList[i]->Cover - meanCover[i]), 2.0);
            }
        }
    }
    for (int i = 0;i < Shrubs;i++){
    ...
    if (day == growEnd) {
        for (int i = 0; i < PFTs; i++){
            ...
            sumGrowth[i] = sumGrowth[i]/(numcells); //_dirk summiert Wachstum uebers ganze Jahr und mittelt auf die Zelle
        }
	}
    ...
    if ((day%vegTimeStep == vegTimeStep-1) && (day >= wetStart) && (day <= wetEnd)){
        for (int i = 0; i < PFTs; i++){
            if (i < Shrubs + Perennials) {
 ...
 * is called in --> VegetationLandscape::calculateProcesses
 *******************************************************************************************/
void VegetationLandscape::writeYearlySummary(WaterLandscape* waterlandscape, Weather* we, int year){/*Tong, based on PFT-oriented*/
    ...
    for (int i = Shrubs; i < Shrubs + Perennials; i++){
        yearlySummaryFile << meanCover[i] << "\t" << meanB[i] << "\t" << grest[i] << "\t"<< grazedPftBM[i] << "\t" << meanT1[i]  << "\t" <<   meanT2[i]  <<  "\t";
    }
    for (int i = 0; i < Shrubs; i++){
         yearlySummaryFile << meanCover[i] << "\t" << MaxSC[i] << "\t" << MinSC[i] << "\t" << SDSC[i] << "\t" << meanB[i] << "\t" << grest[i] << "\t"  << grazedPftBM[i]  << "\t" << ScoverNr[i] << "\t" <<
         estCount[i] << "\t" << Cabove[0][i]  << "\t" << Cabove[2][i]  << "\t" <<  Cabove[9][i] << "\t" << meanT1[i] << "\t" << meanT2[i]  <<  "\t";
    }
    for (int i = Shrubs + Perennials; i < PFTs; i++){
        yearlySummaryFile << meanCover[i] << "\t" << meanB[i] << "\t" << grest[i] << "\t" << grazedPftBM[i] << "\t" << meanT1[i]  << "\t";
    }

    yearlySummaryFile << meanGtotalcover << "\t" << meanStotalcover << "\t" << meanAtotalcover << "\t" << sCsumabove[0] << "\t" << meanRCover << "\t"<< realstock  << "\t" << meanRB << "\t" << grazedBM << "\t"<< we->precSumYear[year] << "\t" << meanE << "\t"
    << meanTL1 << "\t" << meanTL2 << "\t"<< meanMoistSeasonL1 << "\t" << meanMoistSeasonL2<< "\t" << calculateRichness() << "\t" << calculateShannon()  << "\t" << calculateEvenness() <<  "\n";
}
...
void VegetationLandscape::initialiseVegCover() {/*Tong, based on PFT-oriented*/
    ...
    for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize; j++) {
            ...
            for (int m = 0; m < Shrubs; m++){
                if (zuf < SCoverbase[m]) {
                    double zuf2 = rand() / (double(RAND_MAX) + 1);
                    veggrid[i][j]->pftList[m]->Cover = zuf2 * (MaxSCover[m] - MinSCover[m]) + MinSCover[m];
                    ScoverNr[m]++;
                    ...
                    veggrid[i][j]->cohortAge[m] = rand() % (Maxcohort[m] - Mincohort[m]); //random(99)
                    veggrid[i][j]->cohortAgeFire[m] = 0;
                } else {
                    veggrid[i][j]->pftList[m]->Cover = 0;

                    for (int v = Shrubs; v < Shrubs + Perennials; v++) {
                        veggrid[i][j]->pftList[v]->Cover = PerennialNS * veggrid[i][j]->pftList[v]->cmax;
//                        veggrid[i][j]->pftList[v]->Cover = PcoverNS[v-Shrubs] * veggrid[i][j]->pftList[v]->cmax;
                    }
                    for (int v = Shrubs + Perennials; v < PFTs; v++) {
                        veggrid[i][j]->pftList[v]->Cover = 0;
                    }
 ...
 // Function to read in fixed vegetation cover maps for perennial grasses and shrubs
    
//    string VegshrubcoverName, VegperennialcoverName;
//       ifstream VegshrubcoverFile, VegperennialcoverFile;
//       VegshrubcoverName = "Parameters" + delimiter + "vegshrubcover" + ".txt";
//       VegperennialcoverName = "Parameters" + delimiter + "vegperennialcover" + ".txt";
//       
...
//               cerr << VegshrubcoverName
//                    << " could not be opened shrub cover file!\n";
//               exit(-1);
//           }
//
...
//                   for (int m = 0; m < Shrubs; m++) {
//                   veggrid[i][j]->pftList[m]->Cover = vegshrubcover[i][j];
...
//                   }
//                  }
//    //              cout << veggrid[i][j]->pftList[0]->Cover << "\t";
//               }
//    //              cout << endl;
//           }
...
//               VegperennialcoverFile.open(VegperennialcoverName.c_str(), ios::binary|ios::in);
//
//                   //if something goes wrong...
...
//                   for (int i = 0; i < xsize; i++) {
//                       for (int j = 0; j < ysize; j++) {
//                           VegperennialcoverFile >> vegperennialcover[i][j];
//                           for (int v = Shrubs; v < Shrubs + Perennials; v++) {
//                           veggrid[i][j]->pftList[v]->Cover = vegperennialcover[i][j];
//                           }
//    //                       cout << veggrid[i][j]->pftList[1]->Cover << "\t";
//                       }
//    //                   cout <<endl;
//                   }
 ...
 * is called in --> VegetationLandscape::VegetationLandscape
 *******************************************************************************************/
void VegetationLandscape::initialiseVegParams(Parameter* p) {/*Tong, based on PFT-oriented*/

    //constant of the vegetation model
	double aGrowthConst_, resWater_, moistL1_, moistL2_, overlap_, grazeParam_, MAP_ ,  bm_c_rain_ , grazeparA_ ,  grazeparB_, firecoeff_,   fuelLim_, EnScover_, initialshare_;

    //open vegetation parameters file
  	ifstream parameterFile;
  	string vegetationParametersName;

    vegetationParametersName = "Parameters" + delimiter + "vegetationparameters_" + p->vegetationFileID[p->scenario] + ".txt";

    cout << "Read vegetation parameters from: " << vegetationParametersName << endl;
    ...
    for (int k = Shrubs; k < Shrubs + Perennials; k++) {
        parameterFile.ignore(fsize, '*');    //read parameters from first perennial grass
        parameterFile.ignore(fsize, ':');
        parameterFile >>  name_[k];
        parameterFile.ignore(fsize, ':');
        parameterFile >>  type_[k];
    ...
    for (int k = Shrubs + Perennials; k < PFTs; k++) {
        parameterFile.ignore(fsize, '*');   //read parameters from first annual grass
        parameterFile.ignore(fsize, ':');
        parameterFile >>  name_[k];
        parameterFile.ignore(fsize, ':');
        parameterFile >>  type_[k];
  	...
  	for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize; j++) {
            veggrid[i][j] = new VegetationCell(PFTs, Annuals, Perennials, Shrubs, resWater_, moistL1_, moistL2_, overlap_, grazeParam_, aGrowthConst_, bm_c_rain_, grazeparA_, grazeparB_,
                                           MAP_, firecoeff_, fuelLim_, EnScover_, initialshare_, vegTimeStep, cellsize);

            for (int k = 0; k < Shrubs; k++){
                veggrid[i][j]->pftList[k] = new Shrub(name_[k], type_[k], UptakeRate_[k], RootL1_[k], RootL2_[k], GrowthR_[k], Transcoef_[k], Kcoef_[k], ecoef_[k], temcoef_[k], LAI_C_[k],
                                                      MortR_[k], MortBasic_[k], Grazed_[k], WPWaterL1_[k], WPWaterL2_[k], cmax_[k], overlap_, conversionC_BM_[k], grazeParam_, grazeLim_[k],
                                                      grazeprefer_[k], grazepreferintra_[k], dispFrac_[k], estW_[k], bm_c_rain_, MAP_, grazeparA_, grazeparB_, specLoss_[k], firecoeff_,
                                                      fuelLim_, dist0_[k], distConst_[k], distConst2_[k], reserveLast_[k], reserveThis_[k], vegTimeStep, cellsize);
            }

            for (int k = Shrubs; k < Shrubs + Perennials; k++){
                veggrid[i][j]->pftList[k] = new Perennial(name_[k], type_[k], UptakeRate_[k], RootL1_[k], RootL2_[k], GrowthR_[k], Transcoef_[k], Kcoef_[k], ecoef_[k], temcoef_[k], LAI_C_[k],
                                                          MortR_[k], MortBasic_[k], Grazed_[k], WPWaterL1_[k], WPWaterL2_[k], cmax_[k], overlap_, conversionC_BM_[k], grazeParam_, grazeLim_[k],
                                                          grazeprefer_[k], grazepreferintra_[k], dispFrac_[k], estW_[k], bm_c_rain_, MAP_, grazeparA_, grazeparB_, specLoss_[k], firecoeff_,
                                                          fuelLim_, dist0_[k], distConst_[k], distConst2_[k], reserveLast_[k], reserveThis_[k], vegTimeStep, cellsize);
            }
            ...
            for (int k = Shrubs + Perennials; k < PFTs; k++){
                veggrid[i][j]->pftList[k] = new Annual(name_[k], type_[k], UptakeRate_[k], RootL1_[k], RootL2_[k], GrowthR_[k], Transcoef_[k], Kcoef_[k], ecoef_[k], temcoef_[k], LAI_C_[k],
                                                       MortR_[k], MortBasic_[k], Grazed_[k], WPWaterL1_[k], WPWaterL2_[k], cmax_[k], overlap_, conversionC_BM_[k], grazeParam_, grazeLim_[k],
                                                       grazeprefer_[k], grazepreferintra_[k], dispFrac_[k], estW_[k], bm_c_rain_, MAP_, grazeparA_, grazeparB_, specLoss_[k], firecoeff_,
                                                       fuelLim_, dist0_[k], distConst_[k], distConst2_[k], reserveLast_[k], reserveThis_[k], vegTimeStep, cellsize);
            }
 ...
 * is called in --> VegetationLandscape::VegetationLandscape
 *******************************************************************************************/
void VegetationLandscape::initialiseResultFile(Parameter* p){/*Tong, based on PFT-oriented*/

    yearlySummaryFileName = "Results" + delimiter + p->site + "_" + std::to_string(p->simYears) + "years_yearly_" +
    p->outputFileID[p->scenario] + "_climrep-" + std::to_string(p->climateRepetition + 1) + "_modelrep-" + std::to_string(p->modelRepetition + 1) + ".txt";
    ...
    yearlySummaryFile << "year" << "\t"  << "fire" << "\t" ;

    for (int i = 0; i < Perennials; i++){
        yearlySummaryFile << "meanGCover" << i << "\t"<< "meanGB" << i << "\t"<< "pgrest" << i << "\t" << "grazedGBM" << i << "\t" << "PTranspirationL1"  << i << "\t" << "PTranspirationL2" << i <<  "\t";
    }

    for (int i = 0; i < Shrubs; i++){
        yearlySummaryFile << "meanSCover"  << i << "\t" << "MaxSCover" << i << "\t"<< "MinSCover" << i << "\t"<< "SDSCover" << i << "\t"<< "meanSB" << i << "\t" << "srest"  << i << "\t"<< "grazedSBM"  << i << "\t"
        << "shrubCells" << i << "\t" << "estCells" << i << "\t" << "sc01"  << i << "\t" << "sc1"  << i << "\t" <<   "sc10" << i <<"\t"<< "STranspirationL1" << i << "\t" << "STranspirationL2" << i  <<"\t" ;
    }

    for (int i = 0; i < Annuals; i++){
        yearlySummaryFile<< "meanACover" << i << "\t"<< "meanAB" << i << "\t" << "agrest"  << i << "\t" << "grazedABM" << i << "\t"<< "ATranspirationL1" << i  << "\t";
    }

    yearlySummaryFile<< "meanGtotalcover" << "\t" << "meanStotalcover" << "\t"<<"meanAtotalcover" << "\t"<<"scsum01" << "\t"<< "meanRCover" << "\t"<< "effSR" << "\t"<< "meanRB"<< "\t" << "BMGrazed" << "\t"<< "AnnualRain" << "\t" << "Annualevaporation" << "\t"
    << "AnnualtranspirationL1" << "\t" << "AnnualtranspirationL2" << "\t" << "ML1" << "\t" << "ML2"<<"\t" << "Richness" << "\t" << "ShannonDiv" << "\t"<< "Evenness" << "\n";
}

/*******************************************************************************************
 * is called in --> controller::runSimulation
 *******************************************************************************************/
void VegetationLandscape::finishSimulation(){
 ...
 *******************************************************************************************/
VegetationLandscape::VegetationLandscape(Parameter* p, WaterLandscape* waterlandscape){//constructor has been changed by Tong
