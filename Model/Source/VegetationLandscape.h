 ...
 * \brief   In this file, all functions are described that are relevant for vegetation dynamics in a landscape.
...
class VegetationLandscape{
	private:
		...
		Double1D meandML1;	                                    //!< mean moisture to support life in layer 1 (dryMortality)
		Double1D meandML2;                                      //!< mean moisture to support life in layer 2 (dryMortality)
		...
		Double1D meanEndSeasonCoverG;		                    //!< vegetation cover at the end of the growing or wet season
        ...
        /***************************************************************************************//**
         * \brief Calculate maximum, minimum and standard deviation of shrub cover for the whole grid.
         *******************************************************************************************/
		void calculateShrub();
         ...
         *******************************************************************************************/
		void writeYearlySummary(WaterLandscape* waterlandscape, Weather* we, int year);
    ...
    double vegperennialcover[30][30];           // read in grass matrix
    double vegshrubcover[30][30];               // read in shrub matrix _katja
         ...
         * \param p pointer to parameter object;
         *******************************************************************************************/
		void calculateProcesses(int year, int day, WaterLandscape* waterlandscape, Weather* we, Parameter* p);
        ...
        /***************************************************************************************//**
         * \brief Calculate "diffusive" dispersal for grasses and shrubs: Each plant type has
         * a defined dispersal function (decreasing with distance). Within this distance
         * a small fraction of the plant (e.g. by seeds, clonal) is dispersed.
         * \todo Explain parameters here.
         * \param x1 ???;
		...
		//Public parameters
		Int1D estCount;                                             //!< counter for cells in which establishments of seeds was possible
        ...
        double PerennialNS;                                         // max perennial cover divided by number of perennilas when having                                                           no shrub                                           // Katja 
		...
		double meanMoistSeasonL1;		                            //!< mean soil moisture during 'official' growing season
		double meanMoistSeasonL2;
		Int2D Cabove;                                               //!< _dirk to count number of cells above 10 levels of cover (10%,20%...90%)
		...
		double meanGtotalcover;                                     //!< calculate the mean total cover of all the perennial grass for the whole cells
		double meanStotalcover;                                     //!< calculate the mean total cover of all the shrub for the whole cells
		double meanAtotalcover;                                     //!< calculate the mean total cover of all the annual grass for the whole cells
		...
		Double1D reldemand;                                         //!< relative grazing biomass demand for each PFT                       /*Tong*/
		Double1D ratio_Reserve;                                     /*Tong*/
		double percapability;                                       //!< the amount of the livestock body mass per day for biomass demand
		double BiomassNeed;                                         //!< the related parameter to calculate the biomass per year
		...
		Double1D UptakeRate_, RootL1_, RootL2_, GrowthR_, Transcoef_, Kcoef_, ecoef_, temcoef_, LAI_C_, MortR_,
		       MortBasic_, Grazed_, WPWaterL1_, WPWaterL2_, dispFrac_, cmax_, conversionC_BM_, grazeLim_, grazeprefer_,
		       grazepreferintra_, estW_, specLoss_, dist0_, distConst_, distConst2_, reserveLast_, reserveThis_;
