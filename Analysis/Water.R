...
WaterVals_mean <- cover %>%
  group_by(scenario, climrep) %>%
  summarise_at(vars(wateravailability, totalCover, ML1), funs(mean, sd))
...
WaterVals_mean <- cover %>%
  group_by(scenario, climrep) %>%
  summarise_at(vars(ET, totalCover), funs(mean, sd))
...
WaterVals_mean <- cover %>%
  group_by(scenario, climrep) %>%
  summarise_at(vars(Annualevaporation, AnnualtranspirationL1, totalCover), funs(mean, sd))
