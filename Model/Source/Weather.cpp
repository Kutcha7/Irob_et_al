...
void Weather::meanWeather(Parameter* p){
	...
	for (int day = 0; day < daysPerYear*p->simYears; day++) {
		...
		distEarthSun = 1.0 + 0.033*cos(2.0*M_PI/(daysPerYear*1.0)*(day%daysPerYear+1));
		solDeclination = 0.4093*sin(2.0*M_PI/(daysPerYear*1.0)*(day%daysPerYear+1) - 1.405);
		sunSetHour = acos(-1.0*tan(p->latitude*M_PI/180.0)*tan(solDeclination));
		extraterrRadiation[day] = 15.392*distEarthSun*(sunSetHour*sin(p->latitude*M_PI/180.0)*sin(solDeclination)+cos(p->latitude*M_PI/180.0)*cos(solDeclination)*sin(sunSetHour));
