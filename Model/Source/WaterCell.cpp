 ...
 *******************************************************************************************/
void WaterCell::actLayerEvapotranspiration(double rainTot, double potEP){ /*Tong, based on PFT-oriented*/

	//decreasing function with increasing cover
	double vegfunctionL1 = Vegcoef1 - Vegcoef2 * (cGtotal + cAtotal + cStotal);  /*Tong*/

    double vegfunctionL2 = (double)(PtotRootL2 + StotRootL2);       //This parameter has considered the root fraction and add the part of perennial grass /*Tong*/
	...
	if (potEP != nodata){
		...
		double potL0EP = potEP * (1 - (cStotal + cAtotal + cGtotal) / 2.0) * cos_inclination;
        ...
        ///--------------------------evapotranspiration from soil soil layer----------------------------------------------
		...
		///--------------------------evapotranspiration in upper soil layer-----------------------------------------------
		if (waterL1_rel > WP){
			//constant value evaporates above wp, but a residual water content must always be there
			EPL1 = min(potEP * cos_inclination * vegfunctionL1 * EPfactor,(waterL1_rel - resWater) * depthL1);
		}
		else
			EPL1 = max(0.0,min(potEP * pow((double)waterL1_rel / WP,2) * cos_inclination * vegfunctionL1 * EPfactor,(waterL1_rel - resWater) * depthL1));

		///--------------------------transpiration in lower soil layer----------------------------------------------------
        if (waterL2_rel <= WP)
			EPL2 = max(0.0,min(potEP * pow((double)waterL2_rel / WP,2) * vegfunctionL2 * cos_inclination,(waterL2_rel - resWater) * depthL2));

		else
	        EPL2 = min(potEP * cos_inclination * vegfunctionL2,(waterL2_rel - resWater) * depthL2);
 ...
 * is called in --> WaterLandscape::calculateProcesses
 *******************************************************************************************/
void WaterCell::transpirationupper(){  /*Tong, separation of evapotranspiration*/

	if((cStotal + cGtotal + cAtotal) >= 0 && EPL1 >= 0){
        TL1 = max(0.0,Veg_ETcoef1 * (cStotal + cGtotal + cAtotal) - Veg_ETcoef2) * EPL1;
    }
    else
		TL1=0;
}
 ...
 * is called in --> WaterLandscape::calculateProcesses
 *******************************************************************************************/
void WaterCell::transpirationlower(){  /*Tong, separation of evapotranspiration*/
 ...
 * is called in --> WaterLandscape::calculateProcesses
 *******************************************************************************************/
void WaterCell::totalWater(){

	totWater = waterL1_abs + waterL2_abs;
}
 ...
 *******************************************************************************************/
void WaterCell::setCover(Double1D shrubcover, Double1D grasscover, Double1D annualcover) { /*Tong, array transmission*/

	for (int i = 0; i < PFTs; i++){
        if (i < Shrubs) coverPFT[i] = shrubcover[i];			//between 0 and 1
        else if (i >= Shrubs && i < Shrubs + Perennials) coverPFT[i] = grasscover[i-Shrubs];			//between 0 and 1
        else coverPFT[i] = annualcover[i-(Shrubs+Perennials)];			//between 0 and 1
	}
}
...
void WaterCell::totalCover() {
	...
	for (int i = 0; i < PFTs; i++){
        if (i < Shrubs) cStotal += coverPFT[i];
        else if (i >= Shrubs && i < Shrubs + Perennials) cGtotal += coverPFT[i];
        else cAtotal += coverPFT[i];
    }
}
 ...
 * is called in --> Controller::runSimulation
 *******************************************************************************************/
void WaterCell::setVegUseFactors(Double1D PftUseL1, Double1D PftUseL2){  /*Tong, array transmission*/
...
void WaterCell::setAspectFactor() {
	...
	switch (aspect) {
		case FL: aspectFactor = 1.0; break;
		case NN: aspectFactor = 0.90; break;
		case NE: aspectFactor = 0.95; break;
		case EE: aspectFactor = 0.98; break;
		case SE: aspectFactor = 1.03; break;
		case SS: aspectFactor = 1.10; break;
		case SW: aspectFactor = 1.05; break;
		case WW: aspectFactor = 1.02; break;
		case NW: aspectFactor = 0.97; break;
	}
}
...
void WaterCell::setRootL2(Double1D RootL2_){ /*Tong, array transmission*/
    ...
    for (int i = 0; i < PFTs; i++){
        if (i < Shrubs + Perennials) RootL2[i] = RootL2_[i];
        else RootL2[i] = 0;
    }
}
 ...
 *   				   afterwards Green & Ampt infiltration --> wetting front
 *   2. layer (lower): deep infiltration via root channels
 * is called in --> WaterLandscape::calculateProcesses
 *******************************************************************************************/
bool WaterCell::twoLayerInfiltration(double meanWaterL1) {  /*Tong, based on PFT-oriented*/

	double A,B;			        //!< variables to calculate the cumulative infiltration
	double vegFunction;         //!< function to describe the functional response of infiltration to vegetation
	double shrubFunction; 	    //!< same, exclusively for shrubs
	double a3;				    //!< parameter for FL2, determines infiltration without shrub
	double a1;	                //!< vegFunc(0.0, 0.0) = 0.2; ... at least some infiltration, if there is no vegetation
	double a2;	                //!< vegFunc(0.5, 0.5) = 0.95; ... almost full infiltration at a total cover of 1

    
    
	//infiltration without shrubs is determined by hydraulic conductivity
	if (K_geom < 1) a3 = 0.1;
	else if (K_geom < 10) a3 = 0.5;
	else a3 = 0.8;

   // cout << "Kgeom " << K_geom << endl;
   // cout << "Ku " << Ku << endl;
 	shrubFunction = (a3 + (StotRootL2 + PtotRootL2)) / (1 + (StotRootL2 + PtotRootL2)); //_dirk01112010 to include grasses for infiltration  /*Tong*/
	...
	a1 = 0.013;	    //vegFunc(0.0, 0.0) = 0.2; ... at least some infiltration, if there is no vegetation // previous 0.013
	a2 = 0.067;	    //vegFunc(0.5, 0.5) = 0.95; ... almost full infitration at a total cover of 1 // previous 0.067

vegFunction = (a1 + (StotRootL1 + PtotRootL1 + AtotRootL1)) / (a2 + (StotRootL1 + PtotRootL1 + AtotRootL1));  //adjusted by /*Tong*/
	...
	//else ponding can theoretically occur
		...
		if (wettingFrontFlag ==false) {
             ts = ((Na*suction)/(waterL0_abs/K_geom-1)/waterL0_abs)*(vegFunction);
            // ts = ((Na*suction)/(((waterL0_abs/K_geom)-1)*waterL0_abs))*(vegFunction);
	...
	if ((ts >= 0) && (ts < 1)) { //previous ts < 1
		...
		FL1 = min((A / 2.0 + (A * A + 2 * A * B) / (4.0 * sqrt(h5))) * (vegFunction) + Fs,waterL0_abs);
...
void WaterCell::layerDiffusion(){
	...
	if (waterL1_rel > waterL2_rel) {
		double helpbalance = diffConst * K_geom * ((waterL1_rel - waterL2_rel) * depthL1) / ((depthL1 + depthL2) / 2.0);	//Darcy's Law, Maidment 5.17
		...
		double helpbalance = diffConst * K_geom * ((waterL2_rel - waterL1_rel) * depthL2) / ((depthL1 + depthL2) / 2.0);
		waterL2_abs -= helpbalance;
		waterL1_abs += helpbalance;
		waterL2_rel = waterL2_abs / depthL2;
		waterL1_rel = waterL1_abs / depthL1;
...
WaterCell::WaterCell(int x, int y, double suction_, double Ks_, double waterL1_sat_, double WP_, double FC_, double depthL1_, double depthL2_, double iniWaterL0abs_, double iniWaterL1_,
           double iniWaterL2_, double EPfactor_, double resWater_,double maxFL2_,double maxAmountFL2_,double diffConst_, double TE_, int PFTs_, int Perennials_, int Shrubs_,
           int Annuals_, double Vegcoef1_, double Vegcoef2_, double Veg_ETcoef1_, double Veg_ETcoef2_){
